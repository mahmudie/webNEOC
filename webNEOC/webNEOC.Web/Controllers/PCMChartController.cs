...
namespace webNEOC.Controllers
{
    ...
    [RoutePrefix("PCMChart"), Route("{action=index}")]
    [PageAuthorize()]
    public class PCMChartController : Controller
    {
        public ActionResult Index()
        {
            ViewBag.dataSource = GetPCMData();
            return View();
        }

        public List<PCMData> GetPCMData()
        {
            List<PCMData> list = new List<PCMData>();
            ...
            using (con)
            {
                ...
                {
                    command.CommandText = "SELECT * FROM allPCM";
        ...
        public void ExportToPdf(string TreeGridModel)
        {
            ...
            var DataSource = this.GetPCMData();
        ...
        public void ExportToExcel(string TreeGridModel)
        {
            ...
            exp.Export(obj, DataSource, "PCM_Chart.xlsx", ExcelVersion.Excel2013, new TreeGridExportSettings() { Theme = ExportTheme.FlatSaffron });
        }
