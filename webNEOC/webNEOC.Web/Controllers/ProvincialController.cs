...
namespace webNEOC.Controllers
{
    ...
    [RoutePrefix("Provincial"), Route("{action=index}")]
    [PageAuthorize()]
    public class ProvincialController : Controller
    {
        ...
        public List<ProvincialData> GetLQASData()
        {
            List<ProvincialData> list = new List<ProvincialData>();
            ...
            using (con)
            {
                ...
                {
                    command.CommandText = "SELECT * FROM Coverage_province";
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ProvincialData obj = new ProvincialData();
        ...
        public void ExportToExcel(string TreeGridModel)
        {
            ...
            exp.Export(obj, DataSource, "Provincial_coverage.xlsx", ExcelVersion.Excel2013, new TreeGridExportSettings() { Theme = ExportTheme.FlatAzure });
        }
